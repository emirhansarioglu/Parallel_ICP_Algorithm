...
// Without tiling:
// For 40,000 targets: 40,000 reads × 256 threads = 10,240,000 global memory reads!

// With tiling:
// Threads cooperatively load target points into fast shared memory in tiles
// Then all threads read from shared memory
// Example with 40,000 targets and TILE_SIZE=256:
    // - Number of tiles = 40,000 / 256 = 157 tiles
    // - Each tile loaded ONCE by block, used by ALL 256 threads
    // - Global memory reads: 40,000 (once per target) instead of 10,240,000
    ...
    float3* matched_target, float* distances)
{
    ...
    for (int tile_start = 0; tile_start < n_target; tile_start += TILE_SIZE) {        
        if (threadIdx.x < TILE_SIZE && tile_start + threadIdx.x < n_target) {
            // MEMORY COALESCING: Consecutive threads load consecutive memory addresses (each target holds 12 instead 16bytes, float3 -> float4?)
        ...
        for (int i = 0; i < tile_size; ++i) {
            float3 tgt = tile[i];  // Reading from shared memory (speedup happens here)
            ...
            // This if-statement causes warp divergence
            // Different threads find their minimum at different iterations
            // However, this is difficult to avoid in nearest neighbor search
            if (dist < min_dist) {
                min_dist = dist;
                best_match = tgt;
            }
...
// ============================================================================
// CPU HELPER FUNCTIONS (unchanged from original)
// ============================================================================
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Tile memory usage: " << (TILE_SIZE * sizeof(float3)) / 1024.0 << " KB" << std::endl;
    std::cout << std::endl;
    ...
    for (int iter = 0; iter <= 50; ++iter) {
        ...
        // OPTIMIZATION: Call the tiled version instead of the original
        // This uses shared memory tiling for 3-4× speedup on nearest neighbor search
    ...
    CUDA_CHECK(cudaMemcpy(source_f3.data(), d_source, 
                          n_source * sizeof(float3), cudaMemcpyDeviceToHost));
    savePLY("frames/final_aligned.ply", toPoint3D(source_f3, source_cpu));
